...
const UserInfo = ({ username, chatId, locationName, blocked, handleDeleteUser }: User) => {
    ...
    async function handleBlockUser() {
        ...
        try {
            const resp = await axios.post(`/admin/${status ? "unblockUser" : "blockUser"}`, {
                chatId: chatId
    ...
    return (
        <>
            <tr>
                ...
                <td className="px-6 py-4 whitespace-no-wrap border-b border-gray-200">
                    <span
                        className={`inline-flex px-2 text-xs font-semibold leading-5 text-${status ? "red" : "green"}-800 bg-green-100 rounded-full`}>{status ? "blocked" : "Active"}</span>
                </td>
                ...
                <td
                    ...
                    <div className="flex">
                        <button onClick={handleBlockUser} className="text-indigo-600 hover:text-indigo-900 mx-6">{status ? "unblock" : "block"}</button>
